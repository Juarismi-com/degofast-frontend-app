<template>
    <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
         <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <div v-if="loginFail" class="text-center">Email o contraseña invalido</div>
            <form class="space-y-4 md:space-y-6" action="#" @submit="login">
               ...
               <button type="submit" class="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Ingresar</button>
               <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                     ¿Aún no tienes una cuenta? <NuxtLink class="font-medium text-blue-600 hover:underline dark:text-blue-500" to="/auth/register">Registrarte</NuxtLink>
               </p>
            </form>
         </div>
   </div>
...
export default {
   ...
   methods: {
      async login(e){
         ...
         const res = await useFetch(`${API_BASE_URL}/auth/login`, {
            method: 'post',
            body: {
               ...this.form
            }
