   ...
   <div>
      ...
      <div class="grid grid-cols-4 gap-4 pb-4">
         ...
         <div>
            ...
            >
               <option
                  v-for="(item, index) in puntoExpedicionList"
                  :key="index"
                  :value="item.codigo"
               >
                  {{ item.codigo }}
      ...
      <div v-if="formData?.tipoDocumento == 1">
         ...
         <div class="grid grid-cols-4 gap-4">
            <div>
               ...
               <select
                  v-model="formData.condicion.tipo"
                  id="condicionVenta"
                  :class="INPUT_CLASS.sm"
               >
            ...
            <div
               v-if="
                  formData.condicion.credito && formData.condicion.tipo == '2'
               "
            >
               <label for="condicionCreditoTipo">Tipo de crédito:</label>
               <select
                  v-model="formData.condicion.credito.tipo"
                  id="condicionCreditoTipo"
                  :class="INPUT_CLASS.sm"
               >
            ...
            <div
               v-if="
                  formData.condicion.credito.tipo == '1' &&
                  formData.condicion.tipo == '2'
               "
            >
               <label for="condicionCreditoPlazo">Plazo:</label>
               <input
                  v-model="formData.condicion.credito.plazo"
                  id="condicionCreditoPlazo"
                  type="text"
                  :class="INPUT_CLASS.sm"
                  placeholder="Ejemplo: 30 días, 12 meses, etc"
               />
            </div>

            <div
               v-if="
                  formData.condicion.credito.tipo == '2' &&
                  formData.condicion.tipo == '2'
               "
            >
               <label for="condicionCreditoCuotas">Cuotas:</label>
               <input
                  v-model="formData.condicion.credito.cuotas"
                  id="condicionCreditoCuotas"
                  type="number"
                  :class="INPUT_CLASS.sm"
               />
            </div>
            ...
            >
               <button
                  type="submit"
                  class="text-white bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:outline-none focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
               ...
               <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="(cuota, index) in cuotaTable" :key="index">
                     ...
                     <td class="px-4 py-2 whitespace-nowrap">
                        <input
                           type="date"
                           v-model="cuota.fechaVencimiento"
                           :class="INPUT_CLASS.sm"
                        />
                     </td>
                     <td class="px-4 py-2 whitespace-nowrap">
