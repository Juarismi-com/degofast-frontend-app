      ...
      <div class="flex flex-col gap-4 mt-4 mb-4">
         <div class="flex gap-4">
            <div class="flex-1">
               ...
               <input type="text" v-model="searchQuery.numero" id="documentoNumero"
         ...
         <table class="w-full whitespace-no-wrap">
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
               <tr class="text-gray-700 dark:text-gray-400" v-for="(item, index) in filteredItems" :key="index">
                  ...
                  <td class="px-4 py-3 text-sm">{{ item.concepto }}</td>
...
const buscar = async () => {
   ...
   try {
      if (searchQuery.value.numero) {
         const response = await get(`recibo?page=1&numero=${searchQuery.value.numero}`);
